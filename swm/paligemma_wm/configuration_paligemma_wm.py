...
class PaliGemmaWMConfig(PretrainedConfig):
    ...
    Args:
        ...
        image_token_id (`int`, *optional*, defaults to 256000):
        ...
        action_token_id (`int`, *optional*, defaults to 256001):
            The action token index to encode the action prompt.
    ...
    attribute_map = {
        ...
        "action_token_id": "action_token_index",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Backward compatibility: older configs stored these as `image_token_index` / `action_token_index`.
        # `attribute_map` handles attribute-style access but `from_dict`/`__init__` still see the raw keys in kwargs.
        if "image_token_index" in kwargs:
            image_token_id = kwargs.pop("image_token_index")
        if "action_token_index" in kwargs:
            action_token_id = kwargs.pop("action_token_index")

        self._ignore_index = ignore_index
        self.image_token_index = image_token_id
        self.action_token_index = action_token_id
        ...
        # Paligemma uses a bidirectional mask on the prefix (image + prompt). On the Gemma2 backbone this has to be
        # opted into via `text_config.use_bidirectional_attention` so FlashAttention-2 does not force causal masking.
        if getattr(self.text_config, "use_bidirectional_attention", None) is None:
