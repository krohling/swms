...
@auto_docstring(
    custom_intro="""
    Base class for PaligemmaWM outputs, with hidden states and attentions.
    """
)
class PaligemmaWMModelOutputWithPast(BaseModelOutputWithPast):
    r"""
...
@auto_docstring(
    custom_intro="""
    Base class for PaliGemmaWM causal language model (or autoregressive) outputs.
    """
)
...
def create_causal_mask_mapping(
    ...
    **kwargs,
    ...
    for all kinds of forward passes. PaligemmaWM uses a bidirectional mask on the prompt tokens.
...
class PaliGemmaWMModel(PaliGemmaWMPreTrainedModel):
    ...
    def get_action_features(self, action_values: torch.FloatTensor):
        """Projects the actions into the language model token space with the linear projection matrix.

        Args:
            action_values (torch.FloatTensor): Action tensor`.
        """
        return self.action_projector(action_values)

    def get_placeholder_image_mask(
    ...
    def get_placeholder_action_mask(
        self, input_ids: torch.LongTensor, inputs_embeds: torch.FloatTensor, action_features: torch.FloatTensor
    ):
        """
        Obtains multimodal placeholder mask from `input_ids` or `inputs_embeds`, and checks that the placeholder token count is
        equal to the length of multimodal features. If the lengths are different, an error is raised.
        ...
        if input_ids is None:
            special_action_mask = inputs_embeds == self.get_input_embeddings()(
                torch.tensor(self.config.action_token_id, dtype=torch.long, device=inputs_embeds.device)
            )
            special_action_mask = special_action_mask.all(-1)
        else:
            special_action_mask = input_ids == self.config.action_token_id

        special_action_mask = special_action_mask.unsqueeze(-1)
    ...
    def forward(
        ...
        pixel_values: Optional[torch.FloatTensor] = None,
        action_values: Optional[torch.FloatTensor] = None,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> Union[tuple, PaligemmaWMModelOutputWithPast]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should either be in `[0, ...,
            config.text_config.vocab_size]` or -100 (see `input_ids` docstring). Tokens with indices set to `-100` are ignored
            (masked), the loss is only computed for the tokens with labels in `[0, ..., config.text_config.vocab_size]`.
        action_values (torch.FloatTensor): (batch_size, sequence_length, action_dim) - the action values to be projected into the language model token space. 
            These are padded with infinity values to the sequence length. Values of infinity are ignored by the model.
        Example:
        ...
        if pixel_values is not None:
            ...
            special_image_mask = self.get_placeholder_image_mask(
        ...
        if action_values is not None:
            ...
            special_action_mask = self.get_placeholder_action_mask(
                input_ids, inputs_embeds=inputs_embeds, action_features=action_features
            )
        ...
        return PaligemmaWMModelOutputWithPast(
            last_hidden_state=outputs.last_hidden_state,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
            image_hidden_states=image_features if pixel_values is not None else None,
            action_hidden_states=action_features if action_values is not None else None,
        )
...
class PaliGemmaWMForConditionalGeneration(PaliGemmaWMPreTrainedModel, GenerationMixin):
    ...
    # We compute loss with a per-microbatch mean reduction (matching the OLD code), so
    # the Trainer should not pass `num_items_in_batch` and rescale the loss globally.
    accepts_loss_kwargs = False

    def __init__(self, config: PaliGemmaWMConfig):
        super().__init__(config)
        self.model = PaliGemmaWMModel(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if labels is not None:
            # Self contained loss function
            # loss = self.loss_function(
            #     logits=logits, labels=labels, vocab_size=self.config.text_config.vocab_size, **kwargs
            # )
            # original loss function used in my old code 
            ignore_index = getattr(self.config, "_ignore_index", -100)
            if input_ids is not None and self.model.pad_token_id in input_ids:
                labels = torch.where(input_ids == self.model.pad_token_id, ignore_index, labels)
            logits_for_loss = logits.float()
            shift_logits = logits_for_loss[..., :-1, :]
            shift_labels = labels[..., 1:]
            if attention_mask is not None:
                shift_attention_mask = attention_mask[:, -shift_logits.shape[1]:].to(shift_logits.device)
...
__all__ = ["PaliGemmaWMForConditionalGeneration", "PaliGemmaWMPreTrainedModel", "PaliGemmaWMModel"]
