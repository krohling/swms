...
"""
Processor class for PaliGemmaWM.
"""
...
class PaliGemmaWMTextKwargs(TextKwargs):
    suffix: Optional[Union[TextInput, PreTokenizedInput, tp.List[TextInput], tp.List[PreTokenizedInput]]]
...
class PaliGemmaWMProcessor(ProcessorMixin):
    ...
    [`PaliGemmaWMProcessor`] offers all the functionalities of [`SiglipImageProcessor`] and [`GemmaTokenizerFast`]. See the
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not hasattr(tokenizer, "action_token"):
            ...
            self.action_token = tokenizer.action_token
    ...
    def __call__(
        ...
        actions: Union[tp.List[tp.List[Tensor]], tp.List[Tensor]] = None,
        text: Union[TextInput, PreTokenizedInput, tp.List[TextInput], tp.List[PreTokenizedInput]] = None,
        audio=None,
        videos=None,
        **kwargs: Unpack[PaliGemmaWMProcessorKwargs],
        ...
        # Always emit token_type_ids. The model needs them during training (prefix vs. suffix mask), and during
        # generation the prefill-stage mask logic also keys off them when available.
        ...
        if _is_str_or_image(text):
            text = [text]
        elif isinstance(text, tp.List) and _is_str_or_image(text[0]):
            pass
        
        if actions is not None:
            action_values, action_lengths = self._process_actions(actions)
        else:
        ...
        if action_values is not None:
            return_data.update({"action_values": action_values})

        # Only emit `labels` when a suffix was provided. Without a suffix every position has
        # `token_type_ids == 0`, which would yield an all-`-100` labels tensor that the model
        # would still try to compute a (meaningless) cross-entropy on, blowing up memory at
        # eval batch sizes.
        if return_token_type_ids and suffix is not None:
    ...
    def _process_actions(self, inputs: tp.Union[tp.List[torch.Tensor], torch.Tensor, np.ndarray]):
