# adapted from https://github.com/WEIRDLabUW/serl-diffusion
...
from .make_networks import instantiate_model_artifacts
from swm.utils.dataset import normalize_data, unnormalize_data
...
class DiffusionPolicy(object):
    ...
    @staticmethod
    def load(chkpt_path: str, device: str = "cuda"):
        target_device = torch.device(device)
        checkpoint = torch.load(chkpt_path, map_location=target_device, weights_only=False)
        diff_run_config: DiffusionModelRunConfig = checkpoint['config']

        nets, noise_scheduler, device = instantiate_model_artifacts(
            diff_run_config, model_only=True, device=target_device)
    ...
    def sample_trajs(self, num_trajs):
        ...
        obs_cond = obs_cond.repeat(num_trajs, 1)
    ...
    def _process_obs(self, obs: np.ndarray) -> Dict[str, torch.Tensor]:
        ret = {}
        if self.config.with_image:
            # Convert the image to be normalized and the right dataset type
            if obs.dtype == np.uint8:
                obs = obs.astype(np.float32) / 255.0
            ims = [torch.as_tensor(obs).permute(2, 0, 1).to(self.device)]
            ...
            ret["embed"] = images
        if self.config.with_state:
            raise NotImplementedError("Not yet implemented with state")
