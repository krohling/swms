...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self,
        ...
        if not torch.is_tensor(timesteps):
            # Note: constructing a tensor here may trigger a CPU↔GPU sync; prefer passing a tensor timestep.
...
class SimpleDiffusionMLPModel(nn.Module):
    """Simple MLP for diffusion when no temporal modeling is needed (single timestep)."""
    
    def __init__(self, input_dim, global_cond_dim, hidden_dim=512, diffusion_step_embed_dim=256):
        super().__init__()
        ...
        # Simple MLP that processes single timestep
    ...
    def forward(self, sample, timestep, global_cond=None):
        """
        sample: (B, T, input_dim) where T=1 for single timestep
        timestep: (B,) or int
        global_cond: (B, global_cond_dim)
        output: (B, T, input_dim)
        """
        ...
        # Flatten sample for MLP processing
        sample_flat = sample.view(B * T, C)  # (B*T, C)
        cond_expanded = cond.unsqueeze(1).expand(-1, T, -1).reshape(B * T, -1)  # (B*T, cond_dim)
        
        # Concatenate input and conditioning
        input_with_cond = torch.cat([sample_flat, cond_expanded], dim=-1)
        
        # Process through MLP
        output_flat = self.net(input_with_cond)
